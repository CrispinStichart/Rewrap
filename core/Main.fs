...
let maybeChangeWrappingColumn docState rulers =
...
let maybeAutoWrap file settings newText (pos: Position) (getLine: Func<int, string>) =
  ...
  // If column < 1 we never wrap
  elif settings.column < 1 then
    noEdit
  elif not (String.IsNullOrWhiteSpace(newText)) then
    noEdit
  else
    ...
    if not enterPressed && newText.Length > 1 then
      noEdit
    else
      ...
      if visualWidth <= settings.column then
        noEdit
      else
