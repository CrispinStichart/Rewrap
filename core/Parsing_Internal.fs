...
// position
let indentLength: Line -> int =
  Line.onContent (fun s -> s.Length - s.TrimStart().Length)

// Adjusts the line splitpoint so that all whitespace is trimmed from the content
let trimWhitespace: Line -> Line = Line.trimUpTo Int32.MaxValue
...
let container
  : ContentParser -> PrefixTransformer -> (Line -> Option<Line>) -> ContentParser =
  fun content prefixFn lineTest ctx ->
    ...
    let rec wrapFLR: FirstLineRes -> FirstLineRes =
      ...
      | Finished r ->
        Finished(wrapResultParser (fun p -> Some(flpWrapper r.isDefault p)) r)
...
let ignoreAll: ContentParser =
  let rec parseLine line =
    pending line noWrapBlock (ThisLine << parseLine)
