...
/// parser.
type DecorationLine(basedOn: Line) =
  inherit Line(basedOn.prefix, basedOn.content)
  ...
  fun contentParser fmt extraStartLine extraEndLine ctx ->
    ...
    let (lines: Line seq), prefixRegex, initialIndent =
      ...
      | MultiLineBlockFmt(_, tLines, _, info) ->
        let bm =
          if info.bodyMarkers <> "" then
            "[" + info.bodyMarkers + @"]?\s*"
          else
            ""

        tLines :> seq<Line>, regex (@"^\s*" + bm), 0
      | SingleLineBlockFmt(_, info) -> Seq.empty, wsRegex, 0
    ...
    let lines, indentIncrease =
      let mapping minIndent (line: Line) =
        ...
        if line.content.Length = m.Length then
          ...
          line, min minIndent (strWidth m.Value)
        else
          DecorationLine(line) :> Line, minIndent
    ...
    let lines, maybeBodyPrefix =
      let rec adjust: Line -> Line * Option<string> =
        function
        ...
        | _ as line ->
          let line =
            splitAtWidth tabWidth initialIndent indentIncrease line
            |> tabsToSpacesContent tabWidth

          line, if Line.isBlank line.content then None else Some line.prefix

      let mapping maybePrefix =
        adjust >> rmap (fun mlp -> maybePrefix <|> mlp)
    ...
    let desiredBodyPrefix =
      ...
      | SingleLineBlockFmt(line, info) ->
        ...
        if not info.isCStyle || (not addStars && not align) then
          bodyPrefix
        else
          ...
          let baseIndent =
            if align then
              if extraStartLine.IsSome || line :? DecorationLine then
        ...
        let last: seq<Line> =
          maybe
            ...
            (fun l ->
              ...
              Seq.singleton (upcast DecorationLine(l)))
            mbLastLine
    ...
    let lines =
      ...
      | SingleLineBlockFmt(_, info) ->
        if
          info.isCStyle
          && (ctx.settings.blockCommentAddAsterisks
              || ctx.settings.blockCommentAlignWithFirstLine)
        then
          let (Nonempty(first, rest)) = lines
          ...
          let rest =
            rest
            |> List.map (fun line ->
              if line :? DecorationLine then
                line
              else
                line |> Line.mapPrefix (fun _ -> desiredBodyPrefix))
    ...
    | Some line ->
      let line =
        ...
        | MultiLineBlockFmt(_, _, _, info)
        | SingleLineBlockFmt(_, info) ->
          if info.alignEndMarkerToBody then
            adjustEndLine line
          else
            line
        | LineFmt _ -> line
...
type private LineCommentBlock(contentParser: ContentParser) =
  ...
  override _.output ctx lines =
    let fmt =
      LineFmt(lines, strWidth ctx.settings.tabWidth (Nonempty.head lines).prefix)
...
let lineComment: string -> ContentParser -> TryNewParser =
  fun marker contentParser ->
    ...
    let tryMatchLine line =
      tryMatch rx line <|>> fun m -> Line.adjustSplit m.[0].Length line
      ...
      let rec readRestOfBlock blockPrefixWidth =
        let rec readLine line =
          ...
          | Some line ->
            ...
            if linePrefixWidth = blockPrefixWidth then
              ThisLine <| pending line comment readLine
            else
              FinishedOnPrev
              <| Some(pending line comment (readRestOfBlock linePrefixWidth))
          | None -> FinishedOnPrev None

        readLine

      tryMatchLine
      >> map (fun l -> pending l comment (readRestOfBlock (strWidth l.prefix)))
...
type private BlockCommentBlock
  ...
  override _.output ctx (Nonempty(hLine, tLines)) =
    ...
    let inline hasTextUpTo p (str: string) =
      Line.containsText (str.Substring(0, min str.Length p))
    ...
    let mkFirstLine (line: Line) p : Line =
      if not (hasTextUpTo p line.content) then
        upcast (DecorationLine line)
      else
        tabsToSpacesContent ctx.settings.tabWidth line
    ...
    let fmt, extraEndLine =
      ...
      | None ->
        if
          ...
          && not (hasNonWhitespaceAfter (mEndIndex + mEndLen) hLine.content)
        then
          let truncated =
            Line(hLine.prefix, hLine.content.Substring(0, mEndIndex).TrimEnd())
      ...
      | Some tail ->
        let bodyLines, nonTextLastLine, alignEndToBody, extraEndLine =
          ...
          if
            ...
            && not (hasNonWhitespaceAfter (mEndIndex + mEndLen) lastLine.content)
          then
            ...
            let endLine =
              Line("", indent + lastLine.content.Substring(mEndIndex, mEndLen))

            List.rev (bodyLine :: bodyRev), None, true, Some endLine
          elif hasTextUpTo mEndIndex lastLine.content then
            tLines, None, false, None
          else
            List.rev bodyRev, Some lastLine, false, None
...
/// Block comment parser
let blockComment: (string * string) -> (string * string) -> ContentParser -> TryNewParser =
  ...
  fun (bodyMarkers, defaultBodyMarker) (startMarker, endMarker) contentParser ->
    ...
    let onFindStart (startLine: Line) (m: string[]) : FirstLineRes =
      ...
      let pre =
        if m.Length = 1 || m.[1] = "" then
          m.[0]
        else
          m.[0].Replace(m.[1], String.replicate m.[1].Length " ")
      ...
      let rec testForEnd (line: Line) : FirstLineRes =
        ...
        if m.Success then
          finished_ line (comment prefixStart prefixLen baseIndent m.Index m.Length)
        else
          pending line defComment (ThisLine << testForEnd)
...
let sourceCode: List<TryNewParser> -> DocumentProcessor =
  fun commentParsers ->
    let contentParser =
      (tryMany commentParsers |? fun _ line -> finished_ line noWrapBlock)
