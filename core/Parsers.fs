...
let dartdoc: ContentParser -> ContentParser =
  fun content ctx ->
    ...
    let tryMatchTag: Line -> Option<FirstLineRes> =
      ...
      tryMatch' (regex @"^\s*(@nodoc|\{@template|\{@endtemplate|\{@macro)")
      <>>> fun (_, line) -> finished_ line noWrapBlock
  ...
  fun _ctx ->
    let rec parseNext (line: Line) =
      if isBlankLine line || line.content[0] = ' ' || line.content[0] = '\t' then
        ...
        ThisLine ^| pending line wrapBlock parseNext

    fun line ->
      if isBlankLine line || line.content[0] = ' ' || line.content[0] = '\t' then
        finished_ line noWrapBlock
      else
        pending line wrapBlock parseNext
...
let private jsdoc: ContentParser -> ContentParser =
  fun content ctx ->
    ...
    and flpWrapper maybeInnerParser : FirstLineParser =
      freezeInlineTags
      >> (tryMatchTag |? (maybeInnerParser |? content ctx))
      >> wrapFLR
...
let plainText_indentSeparated: ContentParser =
  fun _ctx ->
    ...
    >> fun (indent, line) ->
      if isBlankLine line then
        ...
        pending line wrapBlock (parseNext indent)
