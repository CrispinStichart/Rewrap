...
let private defaultPara (_ctx: Context) : FirstLineParser =
  ...
  let rec parseLine (line: Line) : FirstLineRes =
    ...
    if isMatch lineBreakEnd line then
      Finished(LineRes(line, wrapBlock, true, None))
    else
      Pending(LineRes(line, wrapBlock, true, parseLine >> ThisLine))
...
let private atxHeading: TryNewParser =
  fun _ctx line ->
    if isMatch (mdMarker "#{1,6} ") line then
      Some(finished_ line noWrapBlock)
    else
      None


let private setextUnderline: TryNewParser =
  fun _ctx ->
    let rx = mdMarker @"(?:=+|-+)\s*$"
...
let private fencedCode: TryNewParser =
  fun _ctx ->
    ...
    let tryStart rxStart rxEnd line =
      tryMatch rxStart line
      |> map (fun m -> pending line noWrapBlock (parseLine m.[1].Length rxEnd))
...
let private htmlType1To6: TryNewParser =
  fun ctx ->
    let rec matchEnd (rx: Regex) fromPos (line: Line) : FirstLineRes =
      if rx.Match(line.content, fromPos).Success then
        finished_ line noWrapBlock
      else
        pending line noWrapBlock (matchEnd rx 0 >> ThisLine)
    ...
    let mkParser (patStart, patEnd) =
      ...
      fun _ctx (line: Line) ->
        ...
        if not m.Success then
          ...
          Some <| matchEnd rxEnd m.Length line
    ...
    let mkParserText (patStart, endText) =
      ...
      let rec matchEndText fromPos (line: Line) : FirstLineRes =
        if line.content.IndexOf(endText, fromPos, StringComparison.Ordinal) >= 0 then
          ...
          pending line noWrapBlock (matchEndText 0 >> ThisLine)
      ...
      fun _ctx (line: Line) ->
        ...
        if not m.Success then
          ...
          Some <| matchEndText m.Length line
...
let private indentedCode: TryNewParser =
  fun _ctx ->
    let rec parseLine line =
      if not (isBlankLine line) && indentLength line < 4 then
        FinishedOnPrev None
      else
        ThisLine(pending line noWrapBlock parseLine)
...
let private thematicBreak: TryNewParser =
  fun _ctx ->
...
let private blockquote: ContentParser -> TryNewParser =
  fun getNewBlock ctx ->
    let findMarker line : Option<Line> =
      tryMatch (mdMarker "> ?") line
      |> map (fun c -> Line.adjustSplit (c.[0].Length) line)
...
let private footnote: ContentParser -> TryNewParser =
  fun getNewBlock ctx ->
    let testLineIndent (line: Line) : Option<Line> =
      ...
      if not (isBlankLine line) && indentLength line < minIndent then
        None
      else
        Some(Line.adjustSplit minIndent line)
...
let private listItem: ContentParser -> TryNewParser =
  fun getNewBlock ctx ->
    ...
    let testLineIndent childIndent line =
      if not (isBlankLine line) && indentLength line < childIndent then
        None
      else
        Some(Line.adjustSplit childIndent line)
    ...
    fun line ->
      option {
        ...
        let childIndent = // If spaces after marker > 4 we trim it down to 1
          if m.[2].Length <= 4 then
            m.[0].Length
          else
            m.[0].Length - m.[2].Length + 1
...
let private paraInterrupters
  (gnb: ContentParser)
  : List<Context -> Line -> Option<FirstLineRes>> =
  [ atxHeading
    setextUnderline
    thematicBreak
    blockquote gnb
...
let private markdownWith
  (extras: List<Context -> Line -> Option<FirstLineRes>>)
  : ContentParser =
  ...
  let rec getNewBlock: ContentParser =
    fun ctx ->
      ...
      let rec paragraphFallback ctx =
        defaultPara ctx
        >> function
          | Pending r -> Pending(LineRes(r.line, wrapBlock, true, parseOtherLine))
          | Finished r ->
            Finished(LineRes(r.line, wrapBlock, true, Some parseLineAfterLineBreak))
  ...
  and tryParaInterrupter =
    tryMany (blankLine :: extras @ paraInterrupters getNewBlock)

  and tryNewBlock =
    tryMany (blankLine :: extras @ newBlocks getNewBlock tryParaInterrupter)
