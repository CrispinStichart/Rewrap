...
let cons: 'a -> 'a Nonempty -> 'a Nonempty = fun h neList -> h .@ toList neList
...
let last: 'a Nonempty -> 'a = fun (Nonempty(h, t)) -> fromMaybe h (List.tryLast t)
...
let replaceHead: 'a -> 'a Nonempty -> 'a Nonempty = fun h -> mapHead (fun _ -> h)
...
let concatMap: ('a -> 'b Nonempty) -> 'a Nonempty -> 'b Nonempty =
  fun fn neList ->
    let rec loop output =
      function
      | [] -> output
      | x :: xs -> loop (append (fn x) output) xs
...
let splitAt: int -> 'a Nonempty -> ('a Nonempty * 'a Nonempty Option) =
  fun n (Nonempty(head, tail)) ->
    let rec loop count leftAcc maybeRightAcc =
      ...
      | Some(Nonempty(x, xs)) ->
        if count < 1 then
          leftAcc, maybeRightAcc
        else
          loop (count - 1) (cons x leftAcc) (fromList xs)
...
/// returns an Option.
let spanMaybes
  : ('a -> 'b Option) -> 'a Nonempty -> ('b Nonempty * 'a Nonempty Option) Option =
  fun fn ->
    let rec loop output maybeRemaining =
      let inline finish () =
        fromList (List.rev output) |> map (fun o -> o, maybeRemaining)

      match maybeRemaining with
      | Some(Nonempty(h, t)) ->
        match fn h with
        | Some x -> loop (x :: output) (fromList t)
        | None -> finish ()
      | _ -> finish ()
