...
let canBreakBetweenChars c1 c2 =
  if isWhitespace c1 || isWhitespace c2 then
    true
  elif Array.contains c1 cjNoEnd || Array.contains c2 cjNoStart then
    false
  elif isCJ c1 || isCJ c2 then
    true
  else
    false
...
/// between chars
let addSpaceBetweenChars c1 c2 =
  if c1 = 10us || isCJ c1 || isCJ c2 then false else true
...
let concatLines doubleSentenceSpacing lines =
  ...
  let addLine (acc: string) (line: string) =
    ...
    if line = String.Empty || acc = String.Empty then
      acc
    else
      ...
      let space =
        if doubleSentenceSpacing && Array.contains accEnd stops then
          "  "
        else
          " "

      if addSpaceBetweenChars accEnd (uint16 line.[0]) then
        acc + space + line
      else
        acc + line
...
let breakUpString addLine tabWidth maxWidth (str: string) =
  let popOrPeek (Nonempty(first, rest) as list) = first, Nonempty.fromList rest |? list

  /// Gets the width of the first prefix in the given list
  let prefixWidth prefixes =
    (strWidth tabWidth (Nonempty.head prefixes))
  ...
  let rec findBreakPos min p =
    if p = min then
      min
    elif canBreakBetweenChars (uint16 str.[p - 1]) (uint16 str.[p]) then
      p
    else
      findBreakPos min (p - 1)
  ...
  let outputLine prefixes pStart pEnd =
    ...
    let content =
      if pEnd > pStart then
        if str[pEnd - 1] = '\n' then
          str.Substring(pStart, pEnd - pStart - 1)
        else
          str.Substring(pStart, pEnd - pStart).TrimEnd()
      else
        str.Substring(pStart)
  ...
  let rec loop prefixes lineStart curWidth pStr =
    // If we're at the end
    if pStr >= str.Length then
      outputLine prefixes lineStart 0 |> ignore
    else
      ...
      if charCode = 10us then
        ...
        if newWidth <= maxWidth || isWhitespace charCode then
          loop prefixes lineStart newWidth (pStr + 1)
        else
  ...
  fun prefixes -> loop prefixes 0 (prefixWidth prefixes) 0
...
type OutputBuffer(settings: Settings) =
  ...
  member this.skip(lines: string Nonempty) =
    if this.IsEmpty then
      startLine <- startLine + size lines
    else
      this.noWrap lines
  ...
  member _.noWrap(lines: Line Nonempty) =
    let consumed, newOutputLines =
      lines
      |> Seq.fold
        (fun (c, ls) l -> (c + 1, (l.prefix + l.content) :: ls))
        (0, outputLines)
  ...
  member _.emitExtraLine(line: string) = outputLines <- line :: outputLines
  ...
  member _.toEdit() =
    Edit(
      startLine,
      startLine + linesConsumed - 1,
      List.toArray (List.rev outputLines),
      [||]
    )
