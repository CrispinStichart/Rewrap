...
type Bifunctor = Bifunctor
  with
    ...
    static member inline bimap(Bifunctor, f: 'a -> 'b, g: 'c -> 'd, (x: 'a, y: 'c)) =
      (f x, g y)

let inline bimap (f: ^a -> ^b) (g: ^c -> ^d) (x: ^x) =
  ((^x or ^Bifunctor): (static member bimap:
    ^Bifunctor * (^a -> ^b) * (^c -> ^d) * ^x -> ^r) (Bifunctor, f, g, x))

let inline lmap (f: ^a -> ^b) (x: ^x) =
  ((^x or ^Bifunctor): (static member bimap:
    ^Bifunctor * (^a -> ^b) * (^c -> ^d) * ^x -> ^r) (Bifunctor, f, id, x))

let inline rmap (g: ^c -> ^d) (x: ^x) =
  ((^x or ^Bifunctor): (static member bimap:
    ^Bifunctor * (^a -> ^b) * (^c -> ^d) * ^x -> ^r) (Bifunctor, id, g, x))

/// Alt. Can be used to chain Options or functions that return Options
type Alt = Alt
  with

    static member inline alt(Alt, x: Option<'a>, fy: unit -> Option<'a>) =
      Option.orElseWith fy x
    ...
    static member inline alt(Alt, f1: 'a -> bool, f2: 'a -> bool) = fun x -> f1 x || f2 x
    ...
    static member inline alt
      (Alt, f1: 'a -> 'b -> Option<'r>, f2: 'a -> 'b -> Option<'r>)
      =
      fun a b ->
        match f1 a b with
        | None -> f2 a b
...
type HasDefault = HasDefault
  with

    static member inline withDefault(HasDefault, td: unit -> 'r, x: Option<'r>) =
      Option.defaultWith td x

    static member inline withDefault(HasDefault, d: 'r, x: Option<'r>) =
      Option.defaultValue d x
    ...
    static member inline withDefault
      (HasDefault, fd: 'a -> 'b -> 'r, f: 'a -> 'b -> Option<'r>)
      =
      fun a b ->
        match f a b with
        | None -> fd a b
...
let inline orElse (d: ^d) (x: ^x) =
  ((^x or ^HasDefault): (static member withDefault: ^HasDefault * ^d * ^x -> ^d) (HasDefault,
                                                                                  d,
                                                                                  x))
...
let inline maybe' (tb: unit -> 'b) (f: 'a -> 'b) (x: Option<'a>) : 'b =
  fromMaybe' tb (map f x)
  ...
  let rec safeSkip: int -> 'a list -> 'a list =
    fun n list ->
      if n = 0 then
        ...
        tryTail list |> maybe [] (safeSkip (n - 1))
  ...
  let spanMaybes: ('a -> 'b Option) -> 'a list -> 'b list * 'a list =
    fun fn ->
      let rec loop acc =
        ...
        | h :: rest ->
          match fn h with
          | Some x -> loop (x :: acc) rest
          | None -> List.rev acc, h :: rest
        | [] -> List.rev acc, []
