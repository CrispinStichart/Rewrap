...
let inline private regex str = Regex(str, RegexOptions.IgnoreCase)

let private scriptMarkers = (regex "<script", regex "</script>")

let private cssMarkers = (regex "<style", regex "</style>")
...
let sgml
  ...
  let otherParsers =
    tryMany
      [ blankLines
        Comments.blockComment markdown ("", "") ("<!--", "-->") settings
  ...
  let isBlockTag (pattern: string) (line: string) =
    ...
    m.Success
    && (Seq.isEmpty blockTags || Seq.contains (m.Groups.[1].Value.ToLower()) blockTags)

  let beginsWithBlockStartTag = isBlockTag @"^\s*<([\w.-]+)"
  let justBlockEndTag = isBlockTag @"^\s*</([\w.-]+)\s*>"
  let endsWithBlockTag = isBlockTag @"([\w.-]+)>\s*$"

  let breakBefore line =
    justBlockEndTag line || beginsWithBlockStartTag line
