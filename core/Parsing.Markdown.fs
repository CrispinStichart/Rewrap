...
let rec markdown (settings: Settings) : TotalParser<string> =
  ...
  let fencedCodeBlock (Nonempty(headLine: string, tailLines: List<string>) as lines) =
    ...
    let takeLinesTillEndMarker marker (startLineIndent: int) : Lines * Option<Lines> =
      ...
      let contentLines, otherLines = List.span (not << lineStartsWith marker) tailLines
  ...
  /// Ignores tables
  let table =
    let cellsRowRegex = Regex("\\S\\s*\\|\\s*\\S")

    let dividerRowRegex = Regex(":?-+:?\\s*\\|\\s*:?-+:?")
    ...
    let splitter lines =
      ...
      | firstLine :: secondLine :: rest ->
        ...
        then
          ...
          |> Tuple.mapFirst (fun rows -> Nonempty(firstLine, secondLine :: rows))
  ...
  let nonText =
    ignoreParser (Nonempty.span (fun s -> not (Line.containsText s || Line.isBlank s)))

  /// Ignores ATX headings (### Heading ###)
  ///
  /// A space is required in the commonmark spec, though not in all others.
  let atxHeading = ignoreParser (Nonempty.span (lineStartsWith "#{1,6} "))
  ...
  let blockQuote =
    ...
    let mapper lines =
      ...
      let prefixes, mkDefPrefix =
        if settings.reformat then
          Nonempty.singleton "> ", always "> "
        else
          map fst splitLines, id
  ...
  let indentedCodeBlock =
    let takeLines = Nonempty.span (Line.contains (Regex("^(\\s{4}|\\t)")))
  ...
  let listItem (Nonempty(firstLine, otherLines)) =
    let doStuff listItemPrefix =
      let strippedFirstLine = String.dropStart (String.length listItemPrefix) firstLine
      ...
      let indent = String.length prefixWithSpace
      ...
      let tailRegex = Regex("^ {0," + indent.ToString() + "}")
      ...
      (((headPrefix, (String.replicate (String.length headPrefix) " ")),
        Nonempty(strippedFirstLine, List.map (Line.split tailRegex >> snd) tailLines))
  ...
  let allParsers lines =
    tryMany
      [ blankLines
        ...
        blockQuote ]
      lines
    |> Option.defaultWith (fun () -> takeUntil paragraphTerminator paragraph lines)
...
let private listItemRegex = mdMarker @"([-+*]|[0-9]+[.)])(\s+|$)"

let private blockQuoteRegex = mdMarker ">"

let private fencedCodeBlockRegex = mdMarker "(`{3,}|~{3,})"

let private lineStartsWith = Line.contains << mdMarker
...
let private findListItemEnd
  indent
  : MarkdownState -> List<string> -> List<string> * Option<Lines> =

  let trimIndent (line: string) : string =
    let mutable p = 0

    while p < indent && p < line.Length && line.[p] = ' ' do
      p <- p + 1
