...
/// enough.
let private blockCommands = [| "["; "begin"; "item" |]
...
let private takeFrom2ndLineUntil otherParser parser (Nonempty(headLine, tailLines)) =
  ...
  let bufferToBlocks = Nonempty.rev >> parser

  let rec loopFrom2ndLine buffer lines =
    match Nonempty.fromList lines with
    | None -> (bufferToBlocks buffer, None)

    | Some(Nonempty(head, tail) as neLines) ->
      match otherParser neLines with
      | None -> loopFrom2ndLine (Nonempty.cons head buffer) tail

      | Some result -> result |> Tuple.mapFirst (Nonempty.append (bufferToBlocks buffer))
...
let private findPreserveSection
  beginMarker
  : Nonempty<string> -> Blocks * Option<Nonempty<string>> =
  ...
  let rec loop acc (lines: List<string>) : List<string> * List<string> =
    ...
    | [] -> (List.rev acc, [])
    | head :: tail ->
      if checkLine head 0 then
        (List.rev (head :: acc), tail)
      else
        loop (head :: acc) tail
...
let latex (settings: Settings) : TotalParser<string> =
  ...
  let rec command
    (Nonempty(headLine, tailLines) as lines)
    : Option<Blocks * Option<Nonempty<string>>> =
    ...
    else if trimmedLine.StartsWith("$$") || Array.contains cmdName blockCommands then
  ...
  and plainText: Nonempty<string> -> Blocks =
    ...
    let processChunk =
      (Nonempty.mapTail freezeAnyEOLComment
  ...
  and otherParsers =
    tryMany [ blankLines; Comments.lineComment markdown "%" settings; command ]
