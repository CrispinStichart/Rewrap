...
type private LineRange private (s: int, e: int) =
  ...
  static member toInfinity startLine =
    LineRange(startLine, Int32.MaxValue - 1)
  ...
  let step (mCur: LineRange option, output: LineRange list) (next: LineRange) =
    ...
    | Some cur ->
      if cur.endLine >= next.startLine then
        if cur.isEmpty && next.isEmpty then
          (Some next, output)
          ...
          (Some <| LineRange.fromStartEnd cur.startLine (max cur.endLine next.endLine),
           output)
      else
  ...
  fun context selections parseResult ->
    ...
    let rec loop (sels: List<LineRange>) start (Nonempty(block, otherBlocks)) origLines =
      ...
      | _ ->
        ...
        let nextBlocks =
          maybe otherBlocks (fun b -> b :: otherBlocks) maybeRemainingPartialBlock
        ...
        | Some neNextBlocks ->
          ...
          if nextSels.IsEmpty then
            ...
            let nextOrigLines =
              ...
              | Some lines -> lines
              | None ->
                if nextBlocks |> List.forall (fun b -> size b = 0) then
                  origLines
                else
                  Option.get nextOrigLines
...
let private trimEdit (originalLines: Nonempty<string>) (edit: Edit) : Edit =
  ...
  while s < edit.lines.Length
        && s <= edit.endLine - edit.startLine
        && originalLinesArray[edit.startLine + s] = edit.lines[s] do
    s <- s + 1

  let mutable e = 0

  while e < edit.lines.Length - s
        && e <= edit.endLine - edit.startLine - s
        && originalLinesArray[edit.endLine - e] = edit.lines[edit.lines.Length - 1 - e] do
    e <- e + 1

  Edit(
    edit.startLine + s,
    edit.endLine - e,
    Array.sub edit.lines s (edit.lines.Length - s - e),
    edit.selections
  )
...
let wrapSelected: Nonempty<string> -> Selection seq -> Context -> Edit =
  fun originalLines selections context ->
    ...
    let selectionRanges =
      if Seq.isEmpty selections then
        [ LineRange.toInfinity 0 ]
      else
