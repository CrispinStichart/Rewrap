...
module Native =
  ...
  let files =
    let rec readSpecs dir =
      if not (Directory.Exists(dir + "/docs")) then
        readSpecs (dir + "/..")
      else
        Directory.GetFiles(dir + "/docs/specs", "*.md", SearchOption.AllDirectories)
...
let readTestLines
  ...
  lines
  : Result<Test * Option<Test>, TestError> =
  ...
  let splitAtWidth (col: int) (s: string) : string * string =
    let rec loop i width =
      if i > s.Length || width > col then
        i - 1
      elif i >= s.Length then
        i
      else
        loop (i + 1) (width + Core.strWidth 1 (s.Substring(i, 1)))
  ...
  let getWrappingColumn lines =
    let ps =
      lines
      |> Seq.map (strWidthBefore "¦")
      |> Seq.filter (fun x -> x >= 0)
      |> Seq.toArray

    if ps.Length > 0 && Seq.forall (fun p -> p = ps.[0]) ps then
      Some ps.[0]
    else
      None
  ...
  let getSelections (lines: Lines) : Result<Selection array, TestErrorType> =
    let sel l1 c1 l2 c2 =
      { anchor = { line = l1; character = c1 }
        active = { line = l2; character = c2 } }
    ...
    let rec loop sels pending i line : Result<Selection array, TestErrorType> =
      ...
      if m.Success then
        ...
        | None, x -> loop sels (Some(x, i, c)) i (line.Remove(c, 1))
        ...
        loop sels pending (i + 1) (enum.Current)
      else
  ...
  let lines, only =
    ...
    let findOnly b (l: string) =
      if l.EndsWith(keyword) then
        l.Replace(keyword, ""), true
      else
        l, b
      ...
      | Ok sels ->
        let mkTest forceReformat expectedLines =
          { fileName = fileName
            ...
            selections = sels
            expected = cleanUp expectedLines }
  ...
  let readSettings (line: string) : TestSettings =
    ...
    { language = pick "language" (fun (s: string) -> s.Trim('"')) defaultSettings.language
      ...
      doubleSentenceSpacing =
        pick "doubleSentenceSpacing" bool.Parse defaultSettings.doubleSentenceSpacing
      ...
      blockCommentAddAsterisks =
        pick
          "blockCommentAddAsterisks"
          bool.Parse
          defaultSettings.blockCommentAddAsterisks
      blockCommentOpenOnNewLine =
        pick
          "blockCommentOpenOnNewLine"
          bool.Parse
          defaultSettings.blockCommentOpenOnNewLine
      blockCommentCloseOnNewLine =
        pick
          "blockCommentCloseOnNewLine"
          bool.Parse
          defaultSettings.blockCommentCloseOnNewLine
      blockCommentAlignWithFirstLine =
        pick
          "blockCommentAlignWithFirstLine"
          bool.Parse
          defaultSettings.blockCommentAlignWithFirstLine }
...
let prettyFileName (fileName: string) =
  fileName.Substring(fileName.IndexOf("/docs/") + 6)

/// Prints details of a test failure with input, expected and actual columns
let printFailure (test: Test) (actual: Lines) =
  let settings =
    [ test.language
      if test.settings.tabWidth <> 4 then
        "tabWidth: " + test.settings.tabWidth.ToString()
      else
        ""
      if test.settings.doubleSentenceSpacing then
        "doubleSentenceSpacing: true "
      else
        ""
      if test.settings.reformat then "reformat: true " else ""
      if not test.settings.wholeComment then
        "wholeComment: false"
      else
        ""
      if test.settings.blockCommentAddAsterisks then
        "blockCommentAddAsterisks: true "
      else
        ""
      if test.settings.blockCommentOpenOnNewLine then
        "blockCommentOpenOnNewLine: true "
      else
        ""
      if test.settings.blockCommentCloseOnNewLine then
        "blockCommentCloseOnNewLine: true "
      else
        ""
      if test.settings.blockCommentAlignWithFirstLine then
        "blockCommentAlignWithFirstLine: true "
      else
        "" ]
...
let runTest (test: Test) =
  let getLine =
    Func<int, string>(fun i -> if i < test.input.Length then test.input.[i] else null)

  let file =
    { language = test.language
      path = ""
      getMarkers = Func<CustomMarkers>(fun () -> Core.noCustomMarkers) }

  let edit = Core.rewrap file test.settings test.selections getLine
  let actual = applyEdit edit test.input

  let arrayEqual (a: 'a[]) (b: 'a[]) =
    (a.Length = b.Length && not (Seq.exists2 (fun x y -> x <> y) a b))

  if arrayEqual actual test.expected then
    true
  else
    printFailure test actual
    false
...
let main argv =
  ...
  let inArgv (f: string) =
    argv |> Array.exists (fun a -> f.EndsWith(a))

  let filesToTest =
    Native.files
    |> Seq.map norm
    |> if Array.isEmpty argv then id else Seq.filter inArgv

  let processTest (acc: Results) =
    function
