...
type CommentFormat =
  ...
  | MultiLineBlockFmt of (LineType * Line) * Line List * Line Option * string * BlockCommentOptions
...
let private inspectAndProcessContent :
  ...
  let desiredBodyPrefix =
    match fmt with
      ...
      | SingleLineBlockFmt (typ_line, _, opts) ->
          ...
          if not opts.isCStyle || (not addStars && not align) then bodyPrefix
          else
  ...
  let lines =
    match fmt with
      ...
      | SingleLineBlockFmt (typ_line, _, _) ->
          singleton typ_line
          ...
          if opts.isCStyle && (settings.blockCommentAddAsterisks || settings.blockCommentAlignWithFirstLine) then
            ...
            let rest =
              ...
              |> List.map (fun (typ, line) ->
                  if typ = Normal then (typ, line |> Line.mapPrefix (fun _ -> desiredBodyPrefix))
                  else (typ, line))
            Nonempty(first, rest)
          else lines
      | LineFmt _ -> lines
  ...
  let blocks =
    ...
    | Some line ->
        let line =
          ...
          | MultiLineBlockFmt (_, _, _, _, opts)
          | SingleLineBlockFmt (_, _, opts) ->
              if opts.alignEndMarkerToBody then adjustEndLine line else line
          | LineFmt _ -> line
...
let blockComment :
  ...
  let rec findEnd acc = function
    ...
    | str :: rest ->
      ...
      if m.Success then
        ...
        if closeOnNewLine && hasTextUpTo m.Index str && not (hasNonWhitespaceAfter endPos str) then
          ...
          in List.rev body, last, false, None, rest
  ...
  fun (Nonempty(hStr, tStrs)) -> option {
    ...
    let mkFirstLine (line: Line) p =
      if not (hasTextUpTo p line.content) then Decoration, line
      else
    ...
    let fmt, extraEndLine, linesAfter =
      ...
      if mEnd.Success then
        ...
        if closeOnNewLine && hasTextUpTo mEnd.Index hLine.content && not (hasNonWhitespaceAfter endPos hLine.content) then
          let truncated = Line(hLine.prefix, hLine.content.Substring(0, mEnd.Index).TrimEnd())
          let indent = leadingWhitespace hStr
          let endLine = Line("", indent + hLine.content.Substring(mEnd.Index, mEnd.Length))
          let fl = mkFirstLine truncated truncated.content.Length
          let opts = { isCStyle = isCStyle; alignEndMarkerToBody = true }
          SingleLineBlockFmt (fl, addedLinesPrefix, opts), Some endLine, tStrs
        else
          let opts = { isCStyle = isCStyle; alignEndMarkerToBody = false }
          SingleLineBlockFmt (mkFirstLine hLine mEnd.Index, addedLinesPrefix, opts), None, tStrs
      else
        let bodyLines, nonTextLastLine, alignEndToBody, extraEndLine, linesAfter = findEnd [] tStrs
