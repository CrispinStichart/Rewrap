...
let block (startMarker, endMarker) =
  block' ("", "") (startMarker, endMarker) markdown
...
let mutable languages =
  [ lang "AsciiDoc" "" ".adoc|.asciidoc" plainText
    lang "AutoHotkey" "ahk" ".ahk" <| sc [ line ";"; cBlock ]
    lang "Basic" "vb" ".vb" (oldSourceCode [ customLine xmldoc "'''"; oldLine "'" ])
    ...
    <| sc [ line ";+"; block (@"#\|", @"\|#") ]
    lang "Configuration" "properties" ".conf|.gitconfig|.pylintrc|pylintrc"
    <| configFile
    lang "Crystal" "" ".cr" <| configFile
    lang "CSS" "postcss" ".css|.pcss|.postcss" css
    ...
    lang "Dart" "" ".dart"
    <| sc
      [ line' "///" (dartdoc_markdown)
        line "//"
        block' ("*", " * ") javadocMarkers dartdoc_markdown
        cBlock ]
    lang "Dockerfile" "docker" "dockerfile" <| configFile
    lang "Elixir" "" ".ex|.exs"
    <| sc [ line "#"; block ("@(?:module|type|)doc\s+\"\"\"", "\"\"\"") ]
    ...
    lang
      ...
      "fsharp"
      ".fs|.fsx"
      (oldSourceCode [ customLine xmldoc "///"; cLine; oldBlock (@"\(\*", @"\*\)") ])
    lang "FIDL" "" ".fidl" <| sc [ line "///?" ]
    lang "Go" "" ".go"
    <| sc [ block' ("", "") (@"/\*", @"\*/") godoc; line' "//" godoc ]
    ...
    <| sc [ jsDocBlock; cBlock; line' "//[/!]" jsdoc_markdown; line @"(?://|#)" ]
    lang "HTML" "erb|htmlx|svelte|vue" ".htm|.html|.svelte|.vue" html
    ...
    lang "LaTeX" "tex" ".bbx|.cbx|.cls|.sty|.tex" <| toNewDocProcessor Latex.latex
    ...
    lang "Octave" "" ""
    <| sc [ block ("#\{", "#\}"); block ("%\{", "%\}"); line "##?"; line "%[^!]" ]
    lang "Pascal" "delphi" ".pas"
    <| sc [ block (@"\(\*", @"\*\)"); block (@"\{(?!\$)", @"\}"); line "///?" ]
    ...
    lang
      ...
      ""
      ".zig|.zon"
      (oldSourceCode [ customLine DocComments.javadoc "//[/!]"; cLine ]) ]
...
let private maybeAddCustomLanguage name (markers: CustomMarkers) : Option<Language> =
  ...
  let maybeLine =
    if isInvalid markers.line then
      None
    else
      Some(oldLine (escape markers.line))

  let maybeBlock =
    if isInvalid (fst markers.block) || isInvalid (snd markers.block) then
      None
    else
      Some(oldBlock (map escape markers.block))
...
let languageForFile (file: File) : Option<Language> =
  ...
  if not (String.IsNullOrWhiteSpace(l) || l.Equals("plaintext")) then
    ...
    Seq.tryFind (Language.matchesFilePath file.path) languages
...
let rec select (file: File) : DocumentProcessor =
  ...
  |> Option.orElseWith (fun () ->
    maybeAddCustomLanguage file.language (file.getMarkers.Invoke()))
