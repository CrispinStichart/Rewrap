...
type OptionSplitFunction<'a, 'b, 'c> =
  Nonempty<'a> -> Option<Nonempty<'b> * Option<Nonempty<'c>>>
...
/// lines into blocks
let optionParser
  : OptionSplitFunction<'a, 'b, 'c> -> TotalParser<'b> -> OptionParser<'a, 'c> =
...
let rec tryMany
  : List<OptionParser<'a, 'a>> -> Nonempty<'a> -> Option<Blocks * Option<Nonempty<'a>>> =
  fun parsers lines ->
    match parsers with
    | [] -> None
    | p :: ps ->
      match p lines with
      | None -> tryMany ps lines
      | result -> result
  ...
  fun otherParser totalParser ->
    let rec loop buffer (Nonempty(headLine, tailLines) as lines) =
      ...
      | Some(blocks, remainingLines) ->
        ...
        | None -> (blocks, remainingLines)
      | None ->
        match Nonempty.fromList tailLines with
        | Some neLines -> loop (headLine :: buffer) neLines
        | None -> (Nonempty(headLine, buffer) |> Nonempty.rev |> totalParser, None)
...
let repeatToEnd: PartialParser<'a, 'a> -> TotalParser<'a> =
  fun partialParser ->
    let rec loop blocks lines =
      ...
      | (newBlocks, None) -> Nonempty.appendToList blocks newBlocks
...
let onIndent
  tabWidth
  (Nonempty(firstLine, otherLines))
  : Nonempty<string> * Option<Nonempty<string>> =

  let indentSize =
    Line.leadingWhitespace >> Line.tabsToSpaces tabWidth >> String.length

  let firstLineIndentSize = indentSize firstLine

  otherLines
  |> List.span (fun line -> abs (indentSize line - firstLineIndentSize) < 2)
  ...
  let prefixes =
    if reformat then
      ...
      (Line.leadingWhitespace headLine,
       List.tryHead tailLines |> Option.defaultValue headLine |> Line.leadingWhitespace)
...
let ignoreFirstLine otherParser settings (Nonempty(headLine, tailLines)) : Blocks =
  let headBlock = ignoreBlock (Nonempty.singleton headLine)
...
let indentSeparatedParagraphBlock
  (textType: Wrappable -> Block)
  (lines: Nonempty<string>)
  : Block =

  let prefix = Line.leadingWhitespace (Nonempty.head lines)
...
let takeLinesBetweenMarkers
  ...
  headLine |> Line.tryMatch startRegex |> Option.map takeUntilEndMarker
