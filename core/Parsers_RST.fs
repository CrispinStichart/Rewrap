...
let private contentWidth: Context -> Line -> int =
  fun ctx line ->
    strWidth' (prefixWidth ctx line) ctx.settings.tabWidth (line.content.TrimEnd())
...
let private endCol (ctx: Context) (line: Line) : int =
  ...
  let p = strWidth tabWidth line.prefix in
  p + strWidth' p tabWidth (line.content.TrimEnd())

let private indentWidth (ctx: Context) (line: Line) : int =
  let tabWidth = ctx.settings.tabWidth

  let ws =
    line.content.Substring(0, line.content.Length - line.content.TrimStart().Length)

  strWidth' (strWidth tabWidth line.prefix) tabWidth ws

let private compareIndents ctx line1 line2 =
  prefixWidth ctx line2 - prefixWidth ctx line1
...
let private matchesPuncLine (line: Line) : Option<int * int> =
  ...
  startsWithPunc str
  |> Option.filter (fun s -> Seq.forall ((=) s) str)
  |> voidRight (wsLength, str.Length)


//---------- Leaf blocks ---------------------------------------------------------------//


let private anonymousHyperlink: TryNewParser =
  fun ctx ->
    tryMatch' (regex "^__ \S") >> map ^| fun (_, line) -> finished_ line noWrapBlock


let private bulletItem: TryNewParser =
  fun ctx ->
    fun line ->
      option {
        ...
        return
          bodyElements false ctx (Line.adjustSplit m[0].Length line)
          |> wrapPrefixFn prefixFn
      }


let private docTestBlock: TryNewParser =
  fun ctx ->
    let rx = regex "^\s*>>>(?:\s|$)"

    let rec nextLine indent =
      trimWhitespace
      >> fun line ->
        if line.content = "" || prefixWidth ctx line < indent then
          finishedOnPrev_
        else
          ThisLine ^| pending line noWrapBlock (nextLine indent)

    let onMatch line =
      pending line noWrapBlock (nextLine (prefixWidth ctx line))

    tryMatch' rx >> map ^| (snd >> trimWhitespace >> onMatch)


let private lineBlock: TryNewParser =
  fun ctx ->
    let rx = regex "^(\s*)\|\s+"
...
let private literalIndented (minIndent: int) : TryNewParser =
  fun ctx ->
    ...
    fun line ->
      if indentWidth ctx line > minIndent then
        Some <| pending line noWrapBlock parseTail
      else
        None


let private literalQuoted (indent: int) : TryNewParser =
  fun ctx ->
    let rec parseTail (prefix: char) (line: Line) : NextLineRes =
      if not (line.content.Length > 0 && line.content[0] = prefix) then
        finishedOnPrev_
      else
        ThisLine <| pending line noWrapBlock (parseTail prefix)
    ...
    >> fun (lineIndent, line) ->
      if lineIndent <> indent then
        ...
        startsWithPunc line.content <|>> fun c -> pending line noWrapBlock (parseTail c)


let private numberedItem: TryNewParser =
  fun ctx ->
    let parseRoman: string -> Option<int> =
      ...
      let rec loop (fn: string -> string) acc exp i (str: string) =
        if exp < 0 then
          (if str.Length = 0 then Some acc else None)
        else
          let pat = vals[exp][i]

          if not (str.StartsWith(fn pat)) then
            loop fn acc exp (i - 1) str
          else
            loop fn (acc + pown 10 exp * i) (exp - 1) 9 (str.Substring pat.Length)
    ...
    fun line ->
      option {
        ...
        if m[1].StartsWith "(" && m[1].EndsWith "." then
          return! None
        else
...
let private paragraph (line2DeterminesIndent: bool) : ContentParser =
  fun ctx ->

    let isSameIndent lineX lineY =
      prefixWidth ctx lineX = prefixWidth ctx lineY

    let isMoreIndented lineX lineY =
      prefixWidth ctx lineX > prefixWidth ctx lineY
    ...
    let rec blankLinesBeforeLiteral indent (line: Line) : NextLineRes =
      if isBlankLine line then
        ...
        FinishedOnPrev <| (literalIndented indent <|> literalQuoted indent) ctx line
    ...
    let checkBlankAndLiteral prevLine (line: Line) : Option<NextLineRes> =
      if line.content <> "" then
        ...
        Some
        <| finishedOnPrev (
          pending line noWrapBlock (blankLinesBeforeLiteral (prefixWidth ctx prevLine))
        )
      else
        Some <| finishedOnPrev (finished_ line noWrapBlock)

    let rec checkIndentDifference prevLine line =
      if not (isSameIndent line prevLine) then
        finishedOnPrev_
      else
        ThisLine <| pending line wrapBlock (restOfParagraph line)
    ...
    let checkUnderline prevLine (line: Line) : Option<NextLineRes> =
      if not line2DeterminesIndent && not (isSameIndent line prevLine) then
        None
      else
    ...
    let maybeTitleLine3 headLine prevLine : NextLineParser =
      let altFn _ (line: Line) =
        ...
        if
          sameIndentAsHeadLine && line.content.TrimEnd() = headLine.content.TrimEnd()
        then
          ThisLine <| finished_ line noWrapBlock
        elif isSameIndent line prevLine then
          if sameIndentAsHeadLine then
            ThisLine <| pending line wrapBlock (restOfParagraph line)
          else
            otherLine2 prevLine line
        else
          finishedOnPrev_
    ...
    let maybeTitleLine2 prevLine : NextLineParser =
      let altFn _ (line: Line) =
        ...
        | false, false ->
          ThisLine <| pending line wrapBlock (maybeTitleLine3 prevLine line)
        | false, true -> ThisLine <| pending line wrapBlock (restOfParagraph line)
        | true, false ->
          finishedOnPrev (pending line wrapBlock (maybeTitleLine3 prevLine line))
...
let private tableGrid: TryNewParser =
  fun ctx ->
    let rx = regex "^\+-{3}[-+]*\+\s*$"

    let rec nextLine prevLine =
      trimWhitespace
      >> fun line ->
        if line.content = "" then
          finishedOnPrev ^| finished_ line noWrapBlock
        else
          let otherIndent = compareIndents ctx prevLine line <> 0

          if otherIndent || (line.content[0] <> '|' && line.content[0] <> '+') then
            finishedOnPrev_
          else
            ThisLine ^| pending line noWrapBlock (nextLine line)

    trimWhitespace
    >> tryMatch' rx
    >> map ^| fun (_, line) -> pending line noWrapBlock (nextLine line)


let private tableSimple: TryNewParser =
  fun ctx ->
    let rx = regex "^=+(?:\s+=+)+\s*$"

    let rec afterHeader indent =
      trimWhitespace
      >> fun line ->
        if line.content = "" then
          ThisLine ^| pending line noWrapBlock (afterHeader indent)
        else
          let d = prefixWidth ctx line - indent

          if d < 0 then
            finishedOnPrev_
          elif d = 0 && isMatch rx line then
            ThisLine ^| finished_ line noWrapBlock
          else
            ThisLine ^| pending line noWrapBlock (afterHeader indent)

    let rec afterFirst indent =
      trimWhitespace
      >> fun line ->
        if line.content = "" then
          ThisLine ^| pending line noWrapBlock (afterFirst indent)
        else
          let d = prefixWidth ctx line - indent

          if d < 0 then
            finishedOnPrev_
          elif isMatch rx line then
            ThisLine ^| pending line noWrapBlock (afterHeader indent)
          else
            ThisLine ^| pending line noWrapBlock (afterFirst indent)
    ...
    >> tryMatch' rx
    >> map
       ^| fun (_, line) -> pending line noWrapBlock (afterFirst (prefixWidth ctx line))


let private transitionOrTitle: TryNewParser =
  fun ctx ->
...
let private explicitOther: TryNewParser =
  fun ctx ->
    ...
    let testLine indent line =
      if prefixWidth ctx (trimWhitespace line) > indent then
        Some line
      else
        None

    let rec literalLine test =
      test
      >> function
    ...
    let onMatch (m: string[], line) =
      ...
      | _ ->
        container
          ...
          test
          ctx
          (Line.adjustSplit m[0].Length line)
