...
let contains: Regex -> string -> bool =
  fun regex line -> regex.IsMatch(line: string)
...
let tabsToSpaces (tabSize: int) (str: string) =
  ...
  | x :: xs ->
    xs
    |> List.map (fun s -> s.PadRight((s.Length / tabSize + 1) * tabSize))
  ...
  fun offset tabWidth str ->
    ...
    let rec loop acc i =
      if i >= str.Length then
        ...
        loop (acc + charWidth tabWidth acc ((uint16) str.[i])) (i + 1)
...
type Line(p: string, c: string) =
  ...
  private new(line: Line, splitAt: int) = Line(line.prefix + line.content, splitAt)
  ...
  static member adjustSplit: int -> Line -> Line =
    fun d line ->
      if line.content = "" then
        ...
        Line(line, line.prefix.Length + d)
