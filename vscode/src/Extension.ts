...
async function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    commands.registerTextEditorCommand(
      ...
      rewrapCommentCommand,
    ),
    commands.registerTextEditorCommand(
      "rewrap.rewrapCommentAt",
      rewrapCommentAtCommand,
    ),
    commands.registerTextEditorCommand(
      "rewrap.toggleAutoWrap",
      autoWrap.editorToggle,
    ),
...
async function rewrapCommentAtCommand(editor: TextEditor) {
  let columnStr =
    customWrappingColumn > 0 ? customWrappingColumn.toString() : undefined;

  columnStr = await window.showInputBox({
    prompt:
      "Enter a column number to wrap the selection to. Leave blank to remove wrapping instead.",
    value: columnStr,
    placeHolder: "",
  });
...
function onSaveDocument(e: vscode.TextDocumentWillSaveEvent) {
  ...
  const file = docType(e.document),
    settings = getCoreSettings(editor, (cs) =>
...
const doWrap = (
  ...
  try {
    ...
    const toCol = (cs) =>
      !isNaN(customColumn)
        ? customColumn
        : maybeChangeWrappingColumn(docState, cs);
