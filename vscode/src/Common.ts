...
export function buildEdits(
  ...
  const oldLines = Array(edit.endLine - edit.startLine + 1)
      .fill(null)
  ...
  const eol = doc.eol === vscode.EndOfLine.CRLF ? "\r\n" : "\n",
    oldText = oldLines.join(eol),
    newText = edit.lines.join(eol),
    diffs = fd(oldText, newText);
  let startPos = new vscode.Position(edit.startLine, 0),
    endPos,
    endOffset = doc.offsetAt(startPos),
    offsetDiff = 0,
    newAnchorPos: Position | undefined,
    newActivePos: Position | undefined;
  ...
  if (sel) {
    ...
    const test = (p) =>
      endPos.isEqual(p)
        ? new Position(edit.startLine + endLine, edit.lines[endLine].length)
        : undefined;
  ...
  if (eb)
    edits.forEach((e) =>
      e.newText ? eb.insert(e.range.start, e.newText) : eb.delete(e.range),
    );
  if (saveState)
    saveDocState({
