...
export interface Setting<T> {
  name: string;
  value: T;
  origin: { scope: number; language: string | null };
}
...
export function getEditorSettings(editor: TextEditor): EditorSettings {
  ...
  return {
    ...
    blockCommentAlignWithFirstLine: setting(
      "rewrap.blockComment.alignWithFirstLine",
    ),
...
function getWrappingColumns(
  ...
  const checkColumn = (col: number) =>
    !Number.isInteger(col) || col < 1
      ? Warnings.column(doc, col, 0)
      : col > 120
        ? Warnings.largeColumn(doc, col, col)
        : col;
  ...
  }
  {
    ...
    const rValue = (r): number =>
      checkColumn(r.column != undefined ? r.column : r);
...
const settingWithOrigin =
  (config: WorkspaceConfiguration, lang: string) =>
  <T>(name: string): Setting<T> => {
