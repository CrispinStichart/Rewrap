...
const checkChange = async (e: TextDocumentChangeEvent) => {
  ...
  try {
    ...
    const settings = getCoreSettings(editor, (cs) =>
      getWrappingColumn(file.path, cs),
    );
    ...
    if (!edit.isEmpty)
      return editor.edit((builder) => buildEdits(doc, edit, builder));
  } catch (err) {
    catchErr(err);
  }
};
...
const setDocumentAutoWrap =
  (wsState: Memento, doToggle: boolean) => async (editor: TextEditor) => {
    const settings = getEditorSettings(editor),
      enabled = settings.autoWrap.enabled;
    ...
    const override: boolean = await (async function () {
      ...
      if (doToggle) {
        val =
          val === undefined || val === enabled.value
            ? !enabled.value
            : undefined;
...
export default function (memento: Memento, subscriptions) {
  ...
  const changeIfAffects = (e: ConfigurationChangeEvent) => (ed: TextEditor) =>
    e.affectsConfiguration("rewrap.autoWrap", ed.document) &&
    onChangeEditor(ed);
  const ifActiveDoc = (fn) =>
    window.activeTextEditor && fn(window.activeTextEditor);

  window.onDidChangeActiveTextEditor(
    (e) => e && onChangeEditor(e),
    undefined,
    subscriptions,
  );
  workspace.onDidChangeConfiguration(
    (e) => ifActiveDoc(changeIfAffects(e)),
    undefined,
    subscriptions,
  );
