...
// For data coming in
const pojoPosition = ({ line, character }: Position): Position => ({
  line,
  character,
});
const pojoSelections = (sels: readonly Selection[]): readonly Selection[] =>
  sels.map(({ anchor, active }) => ({
    anchor: pojoPosition(anchor),
    active: pojoPosition(active),
  }));
const pojoDocState = (vscDocState: DocState) => ({
  ...vscDocState,
  selections: pojoSelections(vscDocState.selections),
});
...
export const maybeChangeWrappingColumn = (
  vscDocState: DocState,
  columns: number[],
): number => Main.maybeChangeWrappingColumn(pojoDocState(vscDocState), columns);
...
export const rewrap = (
  docType: DocType,
  settings: Settings,
  unsafeSelections: readonly VscSelection[],
  docLine: (i: number) => string | null,
): Edit =>
  vscEdit(
    Main.rewrap(docType, settings, pojoSelections(unsafeSelections), docLine),
  );
